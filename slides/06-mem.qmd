...
format: 
  revealjs:
    ...
    pdf-page-height: 900
    pdf-page-width: 1600
    footer: "Unitat 5 · Sistemes Operatius (SO) [🏠](/index.html)</a>"
editor: visual
...
## Necessitat de la gestió de la Memòria {.smaller}
...
Una possible solució seria utilitzar memòria secundària i anar fent [swapping]{.alert} de la memòria principal a la secundària. Però, això seria molt lent i poc viable.
:::

## Gestió de la Memòria {.smaller}
...
## Què és el mapa de Memòria? {.smaller}
...
## Regions del mapa de Memòria? {.smaller}
...
:::{.fragment .center-container}
Els Sistemes Operatius **[ofereixen un model de memòria dinàmic]{.alert}** i necessiten **[mantenir una taula de regions per cada procés]{.alert}** format per un nombre variable de regions que es poden afegir o eliminar durant la seva execució.
:::
...
::: notes
En aquest exemple, el programa defineix variables amb diferents àmbits (global, local i arguments de la funció main). A continuació, mostra les adreces de memòria d'aquestes variables utilitzant l'operador & per obtenir la seva adreça. Les adreces de memòria es mostren en format hexadecimal.
:::
...
1. **Espai lògic d'un procés (Relatives):** Adreces de la memòria que contenen el codi, les dades (inicialitzades i no inicialitzades) i la pila del procés. Aquest espai es crea en el moment d'execució del programa. Per exemple, una variable pot ser referenciada com *a 10 bytes des de l'inici de les dades*.
...
3. **Adreces generades per la CPU** (adreces virtuals): Quan un procés executa codi, la CPU genera adreces virtuals (també anomenades adreces lògiques) per accedir a variables, instruccions o la pila. Aquestes adreces no són adreces físiques, sinó referències dins l’espai d’adreces del procés. Aquestes adreces virtuals no poden accedir directament a la memòria RAM; han de ser traduïdes a adreces físiques mitjançant algun mecanisme de reubicació.

## Repte: Traducció d'adreces {.smaller}
...
:::{.fragment .center-container}
**[Repte del SO: Com es tradueixen les adreces lògiques a físiques?]{.alert}**
:::

## Repte: Reubicació de la Memòria {.smaller}
...
## Reubicació estàtica en temps de compilació {.smaller}
...
## Reubicació estàtica en temps de càrrega {.smaller}

El programa conté adreces lògiques/relocatables.
...
## Reubicació dinàmica en temps d’execució {.smaller}
...
## Exemple de reubicació de la Memòria {.smaller}
...
::: notes
1. Compilació: Traducció de lògiques a físiques (Reubicació estàtica - Compilació)
Quan el programa es compila, el compilador converteix les adreces simbòliques (com x i la crida a la funció) en adreces lògiques. Aquestes adreces lògiques no corresponen directament a la memòria física de l'ordinador; són llocs genèrics on el codi fa referència a variables i funcions.

1. Enllaç i càrrega: Traducció de lògiques a físiques (Reubicació estàtica - Càrrega)
Durant la fase d'enllaç (linking), els enllaçadors assignen les adreces físiques concretes al programa objecte generat pel compilador. Aquestes adreces físiques (com 5010 i 5200) es corresponen amb ubicacions físiques dins de la memòria de l'ordinador, però encara no són les que es fan servir directament en l'execució del programa.
Després, el carregador carrega el programa a la memòria, assignant les adreces físiques a ubicacions específiques (per exemple, 100 i 362).

1. Execució: Traducció de lògiques a físiques durant l'execució (Reubicació dinàmica)
Quan el programa s'executa, utilitza adreces lògiques per referir-se a la memòria. Aquestes adreces lògiques (com 34 i 526) es tradueixen en adreces físiques per accedir a la memòria real. Aquesta traducció la fa el sistema operatiu mitjançant tècniques de reubicació dinàmica o gestió de memòria (com paginació o segmentació).
Així, les adreces lògiques que utilitza el programa durant l'execució (com 34 i 526) no corresponen directament a les adreces físiques que es van assignar en el moment de la càrrega (100 i 362), sinó que el sistema les tradueix a mesura que el programa es va executant.
:::

## Mecanismes de protecció {.smaller}
...
## Protecció a nivell de pàgina {.smaller}
...
## Mecanismes de compartició {.smaller}
...
## Models d'organització de la Memòria física {.smaller}
...
## Assignació contigua: *Màquina nua* {.smaller}
...
## Assignació contigua: *Monitor resident* {.smaller}
...
## Assignació contigua: *Particions múltiples* {.smaller}
...
## Polítiques de selecció de partició {.smaller}
...
## Fragmentació de Memòria {.smaller}
...
- **Interna**. Porció de Memòria d'una partició que no utilitza \blueArrow el procés té menys requeriments. *Suposem que assignem una partició de memòria de 1000 bytes a un procés, però el procés només necessita 800 bytes. Els 200 bytes restants es considerarien fragmentació interna*.
...
::: notes
En aquest exemple el primer procés a planificar ocupa 98K per tant la Regió 1 (64K) no ens serveix, però si la regió 2 (128K). En aquesta regió es desaprofiten 30K. El segon procés (264k) no pot anar a la 3er (256K) però si a la 4rt (512K). En aquesta regió es desaprofiten 248K. En total tenim 278K de fragmentació externa.
:::
...
:::{.callout-note title="Observació" .fragment}
...
## Planificació de particions de mida variable {.smaller}
...
## Fragmentació externa en particions de mida variable {.smaller}
...
## Assignació no contigua: *Paginació* {.smaller}
...
## Esquema i Maquinari: paginació {.smaller}

::: {.center-container}
![](../figures/slides/06-mem/paginacio.png)
:::

:::{.callout-note title="Què representa ×n en aquests diagrames?"}
...
## Format Taula de pàgines {.smaller}
...
## Problema 1: Paginació (Solució) {.smaller}
...
## Problema 1: Paginació (II) {.smaller}
...
:::{.callout-note title="Taula de pàgines"}
|Index|Cel·la| V |
|---|---|---|
| 0 |09H| 1 |
| 1 |01H| 1 |
...
## Problema 1: Paginació (III) {.smaller}
...
1. **Número de Pàgina (9 bits):**
   - `015253H` en binari: `0 0001 0101 0010 0101 0011`
   - El primer 9 bits formen el número de pàgina: `0 0001 0101` que en decimal és el 21 ($2^0+2^2+2^4$).

2. **Desplaçament (12 bits):**
   - `015253H` en binari: `0 0001 0101 0010 0101 0011`
...
## Problema 1: Paginació (IV) {.smaller}
...
## Anàlisi de la paginació {.smaller}
...
## Característiques de la paginació {.smaller}
...
## Observacions sobre la paginació {.smaller}
...
## Esquema i Hardware: Segmentació {.smaller}
...
## Problemes de la segmentació {.smaller}

La segmentació és molt semblant amb l'esquema de gestió de Memòria per particions múltiples de mida variable. \blueArrow Té els mateixos problemes que aquest esquema.
...
## Avantatges de la segmentació {.smaller}
...
## Problema 2: Segmentació paginada {.smaller}
...
1. Esquema de traducció.
2. Taula de segments per un procés amb:
   - Codi: 128KB, Dades: 4KB, Dades: 256B, Pila 16KB
3. Calcular l'adreça física per 0:01100h si les entrades a la taula de segments del procés anterior són: 0h, 1000h, 2000h i 1500h.
:::


## Problema 2: Càlculs {.smaller}
...
Disposem d’un sistema de gestió de memòria on la mida de la pàgina és de 512 bytes, la de Memòria és de 2 MB, i la d’una paraula 1 byte. Un procés consta de com a molt 4 segments. La mida màxima d’un segment és 4K. Totes les taules s’implementen en memòria principal. Donats els dos processos següents:
...
::: {.center-container .fragment}
**[No es pot aplicar la segmentació]{.alert}**
:::
...
## Segmentació Paginada (Esquema) {.smaller}

::: {.center-container}
![](../figures/slides/06-mem/problema/4.png){width="50%"}
:::

## Segmentació Paginada (pàgines i segments) {.smaller}

::: {.center-container}
![](../figures/slides/06-mem/problema/5.png){width="85%"}
:::

## Reflexió final  (I){.smaller}
...
## Reflexió final  (II){.smaller}

Tot i això, **Linux sí que utilitza segmentació en casos puntuals**, especialment:
- per accedir a informació específica de cada procés o fil (**FS/GS base**, TLS),
- en modes de compatibilitat de 32 bits,
- i en fases inicials del procés d’arrencada.
