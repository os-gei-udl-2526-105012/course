---
title: "Planificació de processos"
...
Els **processos** presenten necessitats variades a l’hora d’utilitzar els recursos del sistema. En general, la
majoria dels processos passen per **cicles de burst**, alternant entre períodes d’activitat *intensiva d'ús del processador (CPU)*
i períodes d’activitat *d’entrada/sortida (E/S)*. 
...
::: notes
Assumim un sistema operatiu amb un sol processador on un usuari vol executa el procés Firefox. En aquest escenari, el sistema operatiu ha de de gestionar la Memòria Principal, el Processador i el disc. Per fer-ho, l'usuari obre l'aplicació Firefox des de l'espai d'usuari i el sistema operatiu gestiona la seva execució en l'espai del nucli. Per simplificar, considerarem que tots els processos del sistema operatiu s'executen en un altre processador, i que el nucli del sistema operatiu no es veu afectat per la càrrega de treball de l'usuari.
:::
...
::: notes
Per executar l'Aplicació 1 (APP1), el sistema operatiu realitza les següents accions:

1.  **Carrega el procés (APP1) a la Memòria Principal.** (Indicat amb el número 1 a la imatge)
2.  **El Planificador despatxa APP1 al Processador.** (Pas conceptual que porta del número 2 al 3 a la imatge)
3.  **L'usuari interactua amb APP1.** (Representat a l'Espai Usuari amb el número 4)
:::
...
::: notes
Imaginem ara que apareix una segona aplicació (APP2) mentre APP1 encara s'està executant o està a la memòria principal. En un sistema monoprogramat, APP2 hauria d'esperar que APP1 finalitzi per poder començar la seva execució.
:::
...
::: notes
En un sistema monoprogramat, fins que APP1 no finalitza, cap altra aplicació pot utilitzar el processador de manera activa. Per tant APP1 i APP2 no poden conviure en memòria de forma concurrent. En canvi, en un sistema multiprogramat si.
:::
...
:::: notes
Com que hi ha múltiples processos a memòria però la CPU només pot executar-ne un a la vegada, el sistema operatiu utilitza un algorisme de planificació per decidir:

::: non-incremental
1.  Quin procés s'executa (i quin espera).
2.  En quin ordre s'assignen els recursos.

-   **Maximitzar l'ús dels recursos computacionals** (CPU, memòria, disc, xarxa, etc.).
-   **Minimitzar el temps d'espera dels processos**, garantint que els usuaris puguin executar tasques de manera eficient i ràpida.
:::
::::
...
-   Crides al sistema: Quan un procés sol·licita un recurs o canvia el seu estat.
-   Finalització d'operacions d'E/S: Quan un dispositiu completa una operació per a un procés en espera.
:::
...
1.  ↑ *complexitat* del SO → **Canvi de contexts**.
2.  ↑ *cost* → Interrupcions periòdiques.
3.  ↑ la justicia i l'equitat.
:::
...
::: notes
Imagina un conjunt de tasques curtes i llargues, si sempre seleccionem les curtes optimitzarem el **throughput** però a cost del **temps de resposta**.
:::
...
::: notes
L'algorisme FCFS és un dels més senzills: funciona com una cua de persones; el primer que arriba és el primer que s'atén. Els processos s'executen en l'ordre d'arribada, sense considerar prioritat ni durada del burst. Adequat per sistemes previsibles o de lot amb processos de durada similar. Pot generar efecte convoy, on un procés llarg bloqueja els curts, augmentant el temps d'espera.
:::
...
[^5]: Temps de finalització

[^6]: Temps del primer cicle d'execució
...
2. Calcula les següents mètriques de rendiment: Percentatge d'ús de la CPU, Productivitat, Temps d'espera mitjà, Temps de retorn mitjà i Temps de retorn normalitzat mitjà.
1. Quin és l'efecte convoy en aquest exemple? Com afecta les mètriques calculades?

## Exercici 01: FCFS - Solució {.smaller}
...
## Exercici 01: FCFS - Anàlisi {.smaller}
...
| Quantum / Estratègia | P1 | P2  | P3 | P4  | |
| -------------------- | -- | --- | -- | --- | ------------------ |
...
::: notes
Quantum molt petit → molts canvis de context → sobrecàrrega i baix throughput.

Quantum molt gran → es comporta com FCFS → mala resposta per a processos curts.

RR és ideal per a entorns interactius, però el quantum ha de ser triat amb cura per equilibrar latència i eficiència.
:::
...
2. Calcula les següents mètriques de rendiment: Temps d'espera mitjà, Temps de retorn mitjà, Temps de resposta mitjà i Temps de retorn normalitzat mitjà.


## Exercici 02: RR - Solució {.smaller}
...
$$ \tilde{T}_{espera}= \dfrac{\tilde{T}_{espera}{A} + \tilde{T}_{espera}{B} + \tilde{T}_{espera}{C} }{\#processos}=\dfrac{6+5+3}{3}=4.66$$
$$ \tilde{T}_{retorn} = \dfrac{\tilde{T}_{retorn}{A} + \tilde{T}_{retorn}{B} + \tilde{T}_{retorn}{C}}{\#processos}=\dfrac{13+9+5}{3}=9$$
$$ \tilde{T}resposta =\dfrac{\tilde{T}_{resposta}{A} + \tilde{T}_{resposta}{B} + \tilde{T}_{resposta}{C}}{\#processos}=\dfrac{0+0+1}{3}=0.33$$
$$ \tilde{T}retorn_{N} = \dfrac{7/7 + 9/4 + 5/2}{3}=2.2$$   
...
1. Dibuixa el diagrama de Gantt per a l'algorisme de planificació per prioritats (suposant prioritat ascendent -> B,A,C).
2. Calcula les següents mètriques de rendiment: Percentatge d'ús de la CPU, Productivitat, Temps d'espera mitjà, Temps de retorn mitjà i Temps de resposta mitjà.

## Exercici 03: Prioritats - Solució {.smaller}
...
$$ \tilde{T}_{espera} = \dfrac{\tilde{T}_{espera}(A) + \tilde{T}_{espera}(B) + \tilde{T}_{espera}(C)}{\#processos}=\dfrac{4+0+8}{3}=4$$
$$ \tilde{T}_{retorn} = \dfrac{\tilde{T}_{retorn}(A) + \tilde{T}_{retorn}(B) + \tilde{T}_{retorn}(C)}{\#processos}=\dfrac{11+4+10}{3}=8.33$$
$$ \tilde{T}_{resposta} = \dfrac{\tilde{T}_{resposta}(A) + \tilde{T}_{resposta}(B) + \tilde{T}_{resposta}(C)}{\#processos}=\dfrac{0+0+8}{3}=2.66$$
...
## I si coneixem el futur? – SJF i SRTF {.smaller}
...
1.  Pot provocar **inanició**? Sí, qualsevol política que prioritzi una propietat pot provocar inanició.
2.  Pot provocar **convoy**? Sí, qualsevol política no apropiativa pot provocar convoy.
3.  Redueix el temps de retorn? Sí, redueix el temps mitjà de finalització si i només si tots els processos arriben alhora.
4.  Requereix coneixement previ del temps de burst? Sí, és un dels seus inconvenients principals.
...
## Què són les cues multinivell? {.smaller} 
...
## Cues sense Retroalimentació {.smaller}
...
## Cues amb Retroalimentació {.smaller}

1. Els processos s’assignen inicialment a la cua de major prioritat.
2. El moviment entre cues es basa en el comportament durant l'execució, permetent que processos intensius en E/S, de curta durada, es mantinguin a prop de la CPU, mentre que els processos CPU-intensius descendeixin per no monopolitzar els recursos.
...
## Assignació Temporal de la CPU en Cues amb Retroalimentació {.smaller}
...
## Estratègies de Planificació i Contramesures {.smaller}
...
- **Temps de CPU per Cua**: Cada cua rep un percentatge del temps de CPU:
P. ex., cues interactives reben un percentatge més alt per a reduir el temps de resposta.
