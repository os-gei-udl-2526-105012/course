...
format: 
  revealjs:
    ...
    pdf-page-height: 900
    pdf-page-width: 1600
    footer: "Unitat 7 · Sistemes Operatius (SO) [🏠](/index.html)</a>"
editor: visual
...
## R1: Quina és la teva shell? {.smaller}
...
::: notes
Pistes: - `echo $SHELL` - `cat /etc/shells` - Per canviar de shell, pots utilitzar la comanda `chsh` o simplement escriure el nom de la shell a la línia de comandes (per exemple, `bash`, `zsh`, etc.).
...
``` bash
#!/bin/bash
echo "Hola, $USER! Estàs utilitzant la shell: $SHELL"
```
:::
...
::: notes
Pista: - Utilitza la comanda `chmod +x hola.sh` per donar permisos d'execució. - Quan executes amb `bash hola.sh`, estàs invocant explícitament l'intèrpret de comandes `bash` per executar l'script. En canvi, amb `./hola.sh`, el sistema utilitza la línia shebang (si està present) per determinar quin intèrpret utilitzar.
:::
...
## R6: Arguments de línia de comandes {.smaller}
...
## R8: Interacció amb l'usuari amb `read` {.smaller}
